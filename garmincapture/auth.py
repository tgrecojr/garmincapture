...
rejections, MFA, CN domains, token refresh). The library fights that arms race;
we consume it and track the latest version.
...
— login happens here and returns a live client object; the token store is never
under ``BRONZE_ROOT``.

First run is interactive for MFA (the entrypoint wires ``prompt_mfa`` to stdin);
...
def login(settings: Settings, prompt_mfa=_default_prompt_mfa) -> Garmin:
    """Authenticate and return a live ``Garmin`` client.

    Resumes from the token store when possible; falls back to a full
    credentialed login (which re-writes the token store) otherwise. The library
    handles all retry/refresh internally.
    """
    ...
    client = Garmin(
        ...
        retry_attempts=3,      # library default: transient 5xx/network only
        verify_login=True,     # validate token against the API tier
    )

    log.info("garmin_login_start", tokenstore=settings.garmintokens, is_cn=settings.garminconnect_is_cn)
